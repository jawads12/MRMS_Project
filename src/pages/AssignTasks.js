...
const AssignTasks = () => {
  ...
  const fetchData = () => {
    fetch("http://localhost:5498/Task-table")
...
<tbody>
 {data.filter((component) =>
   component.Name.toLowerCase().includes(searchTerm) || component["Assigned Component"].toLowerCase().includes(searchTerm)  ||component["Name"].toLowerCase().includes(searchTerm)
 ).map((component, index) => (
   <tr key={component.TASK_ID}>
     <td>{component.TASK_ID}</td>
     <td>{`${component["Student ID"]}`}</td>
     <td>{component["Name"]}</td>
     <td>{component["Assigned Component"]}</td>
     <td>{component["Asssigned Task"]}</td>
     <td>{component.DEADLINE}</td>
